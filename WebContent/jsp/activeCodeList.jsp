...
<title>链主后台管理</title>
    ...
    <script type="text/javascript">
    	function toPage(index) {
			window.location.href = "userListPage?pageIndex="+index;
		}
    </script>
    <body>
        <div class="x-nav">
                    ...
                    <!-- 
                        ...
                        <div class="layui-card-body layui-table-body layui-table-main">
                            <table class="layui-table layui-form">
                                ...
                                <tbody>
                                <c:forEach items="${pb.list }" var="active">
                                  <tr>
                                    <td>${active.activeid }</td>
                                    <td>
                                    	<c:if test="${active.createtype=='1' }">系统</c:if>
                                    	<c:if test="${active.createtype=='2' }">后台人工</c:if>
                                    </td>
                                    <td>${active.bindphone }</td>
                                    <td>${active.usephone }</td>
                                    <td>${active.activecode }</td>
                                    <td><fmt:formatDate value="${active.usertime }" pattern="yyyy-mm-dd"/></td>
                                    <td>
                                    	<c:if test="${active.status=='1' }">未使用</c:if>
                        ...
                        <div class="layui-card-body ">
                            <div class="page">
                                ...
                                <c:if test="${pb.pageIndex>1 }">
                                	<a class="prev" href="activeCodeList?pageIndex=${pb.pageIndex-1 }">上一页</a>
                                </c:if>
                                <c:if test="${pb.pageIndex==1 }">
                                ...
                                <c:if test="${pb.pageIndex==pb.totalPage}">
                                  ...
                                  <c:if test="${pb.pageIndex<pb.totalPage}">
                                	<a class="next" href="activeCodeList?pageIndex=${pb.pageIndex+1 }">下一页</a>
                                </c:if>
                                </div>
                            </div>
                        </div>
